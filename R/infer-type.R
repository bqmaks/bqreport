...
#' deliberately not the same thing as its storage in R: a count, an indicator
...
#' family. A numeric column is only read as an indicator when every observed
#' value is 0 or 1.
#'
...
#'
#' @return One of `"continuous"`, `"binary"`, `"indicator"`, `"ordinal"`,
#'   `"nominal"`, `"date"`, `"datetime"` or `"unknown"`. `"count"` is only ever
#'   set explicitly.
#' @export
#' @examples
...
#' infer_type(factor(c("mild", "severe"), ordered = TRUE))
...
infer_categorical_type <- function(x, max_levels) {
  ...
  # Logical data that lost its type on import, most often through a CSV.
