...
- `R/analysis-plan.R` — plan_analysis, analysis_plan_row, validate_plan,
  approve_plan.
- `R/analysis-result.R` — только run_analysis (диспетчер по
  analysis_type).
...
- `R/result-accessors.R` — estimates()/contrasts()/tests()/issues() и
  пр.
...
- [`resampled_correlation()`](https://bqmaks.github.io/bqreport/reference/resampled_correlation.md):
  bootstrap ресемплирует веса вместе с наблюдениями; для методов с
  subject id — cluster bootstrap целыми субъектами, перестановки только
  внутри субъекта.
- Spearman CI: SE Bonett–Wright `sqrt((1 + r^2/2)/(n - k - 3))`,
  `ci_method = "fisher_z_bonett_wright"`.
- Методы без inference
...
`analysis_method_chain(id, methods, advance_on)` и
`fallback`/`advance_on` у
[`analysis_function()`](https://bqmaks.github.io/bqreport/reference/analysis_function.md):
продвижение только по объявленным condition classes; члены цепочки
обязаны иметь идентичные effect measure/scale/ exponentiation;
`attempts(result)` хранит каждую попытку; provenance — declared chain,
actual method, `fallback_used`.

Ограничение: членами runtime chain могут быть только пользовательские
[`analysis_function()`](https://bqmaks.github.io/bqreport/reference/analysis_function.md)/[`analysis_method()`](https://bqmaks.github.io/bqreport/reference/analysis_method.md);
встроенные engines — ещё нет.
...
(включает исполняемый three-group custom HC3 workflow),
`examples-gallery`.
...
- [`set_comparisons()`](https://bqmaks.github.io/bqreport/reference/set_comparisons.md)
  дважды для одного predictor сейчас дописывает дубликат в contrast
  registry — рассмотреть replace-семантику;
